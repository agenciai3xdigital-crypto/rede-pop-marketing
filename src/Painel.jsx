...
export default function Painel() {
  ...
  function exportar() {
    ...
    const margem = 15, larg = 180, altPag = 282;
    let y = margem;
    const nova = (h) => { if (y + h > altPag) { pdf.addPage(); y = margem; } };
    L.forEach((linha) => {
      if (linha.startsWith("== ")) {
        nova(12); y += 4;
        pdf.setFont("helvetica", "bold"); pdf.setFontSize(12); pdf.setTextColor(24, 54, 66);
        pdf.text(linha.replace(/==/g, "").trim(), margem, y);
        pdf.setDrawColor(24, 54, 66); pdf.line(margem, y + 1.5, margem + larg, y + 1.5);
        y += 7;
      } else if (linha.startsWith("FICHA DE DIAGN")) {
        pdf.setFont("helvetica", "bold"); pdf.setFontSize(15); pdf.setTextColor(14, 124, 102);
        const t = pdf.splitTextToSize(linha, larg);
        pdf.text(t, margem, y); y += t.length * 6 + 2;
      } else if (linha.trim() === "") {
        y += 3;
      } else {
        pdf.setFont("helvetica", linha.match(/^\d+\./) ? "bold" : "normal");
        pdf.setFontSize(10); pdf.setTextColor(28, 42, 46);
        const t = pdf.splitTextToSize(linha, larg);
        nova(t.length * 4.6);
        pdf.text(t, margem, y); y += t.length * 4.6 + 1;
      }
    });
    // rodapé com paginação
    const total = pdf.getNumberOfPages();
    for (let p = 1; p <= total; p++) {
      pdf.setPage(p); pdf.setFont("helvetica", "normal"); pdf.setFontSize(8); pdf.setTextColor(120, 130, 130);
      pdf.text("Ficha de diagnóstico — " + (loja || "loja") + "  ·  página " + p + " de " + total, margem, 292);
    }
