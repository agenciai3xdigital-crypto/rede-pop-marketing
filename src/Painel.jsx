...
export default function Painel() {
  ...
  function exportar() {
    ...
    const NAVY = [24, 54, 66], PRIM = [14, 124, 102], AMB = [217, 138, 22], RED = [188, 68, 56],
      SUB = [92, 107, 112], INK = [28, 42, 46], LINHA = [226, 230, 225],
      PRIMS = [227, 241, 237], AMBS = [251, 241, 220], REDS = [249, 232, 229];
    const stC = { ok: PRIM, atencao: AMB, critico: RED };
    const stB = { ok: PRIMS, atencao: AMBS, critico: REDS };
    const pdf = new jsPDF({ unit: "mm", format: "a4" });
    const M = 14, W = 182, FIM = 280;
    let y = 0;
    const quebra = (h) => { if (y + h > FIM) { pdf.addPage(); y = M; } };
    const secao = (t) => {
      quebra(14); y += 5;
      pdf.setFillColor(...NAVY); pdf.rect(M, y - 4.5, W, 7, "F");
      pdf.setFont("helvetica", "bold"); pdf.setFontSize(10.5); pdf.setTextColor(255, 255, 255);
      pdf.text(S(t), M + 2.5, y); y += 6.5;
    };
    const texto = (t, opts = {}) => {
      pdf.setFont("helvetica", opts.b ? "bold" : opts.i ? "italic" : "normal");
      pdf.setFontSize(opts.fs || 9);
      pdf.setTextColor(...(opts.cor || INK));
      const ls = pdf.splitTextToSize(S(t), opts.w || W - (opts.x ? opts.x - M : 0));
      quebra(ls.length * (opts.lh || 4.1) + 1);
      pdf.text(ls, opts.x || M, y);
      y += ls.length * (opts.lh || 4.1) + (opts.mb != null ? opts.mb : 1);
    };
    const chip = (st, x) => {
      const rot = stTxt[st];
      pdf.setFontSize(7.2); pdf.setFont("helvetica", "bold");
      const wc = pdf.getTextWidth(rot) + 4;
      pdf.setFillColor(...stB[st]); pdf.roundedRect(x - wc, y - 3.2, wc, 4.6, 1.4, 1.4, "F");
      pdf.setTextColor(...stC[st]); pdf.text(rot, x - wc + 2, y);
    };

    /* ===== PÁGINA 1 — ANÁLISE ===== */
    pdf.setFillColor(...NAVY); pdf.rect(0, 0, 210, 30, "F");
    pdf.setTextColor(255, 255, 255);
    pdf.setFont("helvetica", "normal"); pdf.setFontSize(8.5);
    pdf.text("FRANQUEADORA PET - DIAGNOSTICO DE MARKETING POR LOJA", M, 9);
    pdf.setFont("helvetica", "bold"); pdf.setFontSize(17);
    pdf.text(S(loja || "(loja não selecionada)"), M, 19);
    pdf.setFont("helvetica", "normal"); pdf.setFontSize(8.5);
    pdf.text(S("Gerada em " + new Date().toLocaleString("pt-BR") + "   |   Preenchida por: " + (preenchedor || "-") + "   |   Reunião com: " + (participantes || "-")), M, 26);
    y = 38;

    if (an) {
      secao("1. INDICADORES DO PDV  (" + an.meses.length + " meses - " + an.linhas.toLocaleString("pt-BR") + " linhas de venda)");
      kpis(an).forEach((gr) => {
        texto(gr.grupo.toUpperCase(), { b: true, fs: 8, cor: SUB, mb: 1.5 });
        gr.itens.forEach((k) => {
          quebra(6);
          pdf.setFont("helvetica", "normal"); pdf.setFontSize(9); pdf.setTextColor(...INK);
          pdf.text(S(k.rot), M + 2, y);
          pdf.setFont("helvetica", "bold");
          pdf.text(S(k.val), M + 96, y);
          chip(k.st, M + W);
          pdf.setDrawColor(...LINHA); pdf.line(M + 2, y + 1.6, M + W, y + 1.6);
          y += 5.4;
        });
        y += 1.5;
      });

      secao("2. RECEITA, CMV E MARGEM POR SETOR");
      quebra(6);
      pdf.setFont("helvetica", "bold"); pdf.setFontSize(7.8); pdf.setTextColor(...SUB);
      pdf.text("SETOR", M + 2, y); pdf.text("RECEITA", M + 96, y, { align: "right" });
      pdf.text("% REC.", M + 120, y, { align: "right" }); pdf.text("CMV", M + 146, y, { align: "right" });
      pdf.text("MG. BRUTA", M + W, y, { align: "right" });
      pdf.setDrawColor(...NAVY); pdf.line(M + 2, y + 1.5, M + W, y + 1.5); y += 5.5;
      an.setores.filter((s) => s.pctRec >= 0.5).forEach((s) => {
        quebra(5.2);
        pdf.setFont("helvetica", "normal"); pdf.setFontSize(8.6); pdf.setTextColor(...INK);
        pdf.text(S(s.setor).slice(0, 44), M + 2, y);
        pdf.text(S(fmtR$(s.receita)), M + 96, y, { align: "right" });
        pdf.text(S(fmtP(s.pctRec)), M + 120, y, { align: "right" });
        pdf.setTextColor(...(s.cmv > 65 ? RED : INK));
        pdf.text(S(fmtP(s.cmv)), M + 146, y, { align: "right" });
        pdf.setFont("helvetica", "bold"); pdf.setTextColor(...(s.margem >= 40 ? PRIM : INK));
        pdf.text(S(fmtP(s.margem)), M + W, y, { align: "right" });
        pdf.setDrawColor(...LINHA); pdf.line(M + 2, y + 1.4, M + W, y + 1.4);
        y += 5;
      });

      if (an.totalGruposDup > 0) {
        secao("3. DUPLICADOS PROVÁVEIS NA BASE");
        texto(an.cliDuplicados + " cadastros em " + an.totalGruposDup + " grupos com o mesmo telefone. Unificar antes da migração para o app. Maiores grupos:", { fs: 8.6, cor: SUB, mb: 2 });
        an.gruposDup.slice(0, 8).forEach((g) => {
          texto("- " + g.tel.replace(/^(\d{2})(\d{4,5})(\d{4})$/, "($1) $2-$3") + "  -  " + g.qtd + " cadastros: " + g.nomes.slice(0, 3).join(" / "), { fs: 8.4, x: M + 2, w: W - 2, mb: 0.6 });
        });
      }
    } else {
      texto("Sem CSVs do PDV carregados - indicadores não calculados.", { i: true, cor: SUB });
    }

    /* ===== PREENCHIMENTO ===== */
    pdf.addPage(); y = M;
    secao("4. ENTREVISTA - PERGUNTAS E RESPOSTAS");
    const qsE = perguntas(an);
    qsE.forEach((q, i) => {
      texto((i + 1) + ". " + q.t, { b: true, fs: 8.8, mb: 0.8 });
      texto("Perguntado: " + (feito[q.id] ? "SIM" : "não") + "   |   Resposta: " + (resp[q.id] || "-"), { fs: 8.8, i: !resp[q.id], cor: resp[q.id] ? INK : SUB, x: M + 4, w: W - 4, mb: 2.6 });
    });

    secao("5. PADRÃO OPERACIONAL DE MARKETING DA LOJA");
    PADRAO.forEach((p, i) => {
      const sit = padrao[i] || "não avaliado";
      texto("- " + p.t, { fs: 8.8, mb: 0.4 });
      texto("Situação: " + sit + (obsPadrao[i] ? "   |   " + (sit === "Já faz" ? "Registrado: " : "Definido: ") + obsPadrao[i] : ""), { fs: 8.6, cor: sit === "Implantar" ? AMB : sit === "Já faz" ? PRIM : SUB, b: sit !== "não avaliado", x: M + 4, w: W - 4, mb: 2 });
    });

    secao("6. PRESTAÇÃO DE SERVIÇO MENSAL DA FRANQUEADORA");
    const svc = SERVICOS.filter((_, i) => servicos[i]);
    if (svc.length) svc.forEach((s) => texto("- " + s, { fs: 8.8, mb: 0.8 }));
    else texto("Nenhum serviço selecionado ainda.", { i: true, cor: SUB });

    /* ===== PLANO DE AÇÃO (final) ===== */
    pdf.addPage(); y = M;
    secao("7. PLANO DE AÇÃO DA UNIDADE  (gerado dos indicadores)");
    const acoesExp = planoAcoes(an, padrao);
    acoesExp.forEach((ac, n) => {
      quebra(20);
      texto((n + 1) + ". " + ac.campanha + "   [" + (planoStatus[ac.id] || "A produzir") + "]", { b: true, fs: 10, cor: NAVY, mb: 0.8 });
      texto("Problema: " + ac.problema, { fs: 8.8, cor: RED, b: true, mb: 1.2 });
      ac.demandas.forEach((d) => texto("- " + d, { fs: 8.8, x: M + 4, w: W - 4, mb: 0.6 }));
      texto("Prazo: " + ac.prazo + "   |   Meta: " + ac.meta, { fs: 8.6, b: true, cor: PRIM, mb: 3 });
    });
    const impl = PADRAO.map((p, i) => ({ p, i })).filter(({ i }) => padrao[i] === "Implantar");
    if (impl.length) {
      texto("Itens do padrão a implantar nesta loja:", { b: true, fs: 9.4, cor: NAVY, mb: 1 });
      impl.forEach(({ p, i }) => texto("- " + p.t + " - " + (obsPadrao[i] || "definição pendente (acordar meta com a loja)"), { fs: 8.8, x: M + 2, w: W - 2, mb: 0.8 }));
    }
    if (melhorias) {
      texto("Melhorias rápidas sugeridas (prazo 7 dias):", { b: true, fs: 9.4, cor: NAVY, mb: 1 });
      texto(melhorias, { fs: 8.8, x: M + 2, w: W - 2 });
    }

    const total = pdf.getNumberOfPages();
    for (let p = 1; p <= total; p++) {
      pdf.setPage(p); pdf.setFont("helvetica", "normal"); pdf.setFontSize(7.5); pdf.setTextColor(120, 130, 130);
      pdf.text(S("Ficha de diagnóstico - " + (loja || "loja") + "  -  página " + p + " de " + total), M, 291);
    }
    pdf.save("Ficha_" + (loja || "loja").replace(/[\s\u2014-]+/g, "_") + ".pdf");
  }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: disp, color: C.ink }}>
      ...
      <div style={{ background: C.navy, color: "#fff", padding: "18px 20px" }}>
        <div style={{ maxWidth: 980, margin: "0 auto", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
          ...
          <button onClick={exportar} style={{ background: "transparent", color: "#fff", border: "1px solid rgba(255,255,255,.4)", borderRadius: 8, padding: "10px 14px", fontSize: 14, cursor: "pointer" }}>Baixar PDF</button>
        </div>
      </div>
        ...
        {aba === "servico" && (
          <div>
            ...
            <div style={{ background: C.primarySoft, border: "1px solid " + C.primary, borderRadius: 12, padding: 14, fontSize: 13, marginTop: 8 }}>
              Ao terminar, use <strong>Salvar ficha</strong> (fica registrada aqui no sistema) e <strong>Baixar PDF</strong> para arquivar no Drive da loja junto com a gravação da reunião.
            </div>
          </div>
        )}
      </div>
